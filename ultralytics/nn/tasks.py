...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f,C2f_DBB, C2f_Rep, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Dual}:
        ...
        #         c2 = make_divisible(min(c2, max_channels) * width, 8)
        #     args = [c1, *args[1:]]
        elif m in {CAA}:#添加注意力模块，没有CBAM、eca的，M删除即可
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]       
